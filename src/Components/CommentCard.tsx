...
const CommentCard = ({comment, replyTo} : {comment: Comment, replyTo?: string}) => {
    ...
    const handleUpvote = () => {
        const typeOfComment = replyTo ? "reply" : "comment"
        fetch(`/api/${typeOfComment}/upvote/${comment.id}`,{
            method:"PUT"
        })
    }
    const handleDownvote = () => {
        const typeOfComment = replyTo ? "reply" : "comment"
        fetch(`/api/${typeOfComment}/downvote/${comment.id}`,{
            method:"PUT"
        })
    }
        ...
        <div className={`p-4 rounded-md border flex flex-col ${replyTo && 'ml-6'} bg-white`}>
            ...
            <div className=' flex justify-between'>
                <div className=" bg-veryLightGray flex justify-between w-20 py-1 rounded-md">
                    <button onClick={() => handleUpvote()} className=" text-grayishBlue pl-2 hover:opacity-80 opacity-20 transition-opacity ease-in-out duration-200">+</button>
                    <div className="px-4 font-regular text-modBlue">{comment.score}</div>
                    <button onClick={() => handleDownvote()} className=" text-grayishBlue pr-2 hover:opacity-80 opacity-20 transition-opacity ease-in-out duration-200">-</button>
                </div>
